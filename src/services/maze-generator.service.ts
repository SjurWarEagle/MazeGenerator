...
export class MazeGeneratorService {
  ...
  public generateMaze(width: number, height: number): Maze {
    ...
    maze.finish = maze.cells.find(
      (value) => value.x === width - 1 && value.y === height - 1
    )!;
    if (!maze.finish) {
      throw new Error('no finish point found');
    }
    const currentCell: MazeCell | undefined = maze.cells.find(
      (value) => value.x === maze.begin.x && value.y === maze.begin.y
    );
    if (!currentCell) {
  ...
  distributeLoot(maze: Maze, cnt: number, sideLengthMaze: number): void {
    ...
    for (let i = 0; i < cnt; i++) {
      ...
      const alreadyExists = maze.loot.find(
        (cell) => cell.x === loot.x && cell.y === loot.y
      );
      const tooCloseToOtherLoot = maze.loot.find(
        (cell) =>
          Math.abs(cell.x - loot.x) + Math.abs(cell.y - loot.y) < sideLengthMaze
      );
      const isPartOfIdealPath = maze.wayToExit.find(
        (cell) => cell.x === loot.x && cell.y === loot.y
      );
      const isSpecialField =
        (maze.finish.x === loot.x && maze.finish.y === loot.y) ||
