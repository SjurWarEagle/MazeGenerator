...
export class MazeDisplayComponent implements OnInit {
  ...
  private drawSpecialField(cell: MazeCell): void {
    ...
    const isBegin =
      cell.x === this.localMaze.begin.x && cell.y === this.localMaze.begin.y;
    const isFinish =
      cell.x === this.localMaze.finish.x && cell.y === this.localMaze.finish.y;
    const isPartOfWayToExit =
      this.localMaze.wayToExit.find(
        (pos) => cell.x === pos.x && cell.y === pos.y
      ) !== undefined;
    const isLoot =
      this.localMaze.loot.find(
        (pos) => cell.x === pos.x && cell.y === pos.y
      ) !== undefined;
